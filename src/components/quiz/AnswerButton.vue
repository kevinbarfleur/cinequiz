<template>
  ...
  >
    ...
    <div v-if="userMode === 'host' && assignedTeams.length > 0" class="ml-3 flex-shrink-0 flex items-center gap-2 animate-fade-in-scale">
      <div class="bg-bg-soft text-brand-1 text-xs font-semibold w-6 h-6 rounded-full flex items-center justify-center border border-brand-1/30">
        {{ assignedTeams.length }}
      </div>
      <div class="flex flex-col gap-1 max-w-32">
        <span v-for="teamName in assignedTeamNames" :key="teamName" class="text-xs font-medium text-brand-1 bg-bg-soft px-2 py-1 rounded border border-brand-1/30 whitespace-nowrap overflow-hidden text-ellipsis">
          {{ teamName }}
        </span>
      </div>
    </div>
...
const buttonClasses = computed(() => {
  ...
  classes.push(
    'w-full min-h-12 mobile:min-h-14 p-3 mobile:p-4',
  ...
  if (props.userMode === 'host') {
    ...
    if (props.showResult) {
      if (props.isCorrect) {
        ...
        // Neutral state
        classes.push(
          'bg-bg-soft text-text-3 border-divider opacity-60'
        )
      }
    } else if (props.isSelected) {
      // Selected state
  ...
  classes.push('mobile:min-h-14 mobile:p-5')
