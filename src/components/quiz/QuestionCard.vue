  ...
  <div class="w-full max-w-2xl mx-auto">
    <BaseCard variant="elevated" padding="lg" class="relative">
        ...
        <div class="w-full bg-gray-200 rounded-full h-3 overflow-hidden">
          <div
            ...
            :style="{
              ...
              background:
                'linear-gradient(135deg, var(--vp-c-brand-1), var(--vp-c-brand-next))',
            }"
          ></div>
        </div>
      </div>
      ...
      <div class="mb-8 animate-fade-in-delay">
        <div class="flex flex-wrap items-center gap-2 mb-4">
          <div v-if="question.category">
            <span
              class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-bg-soft text-brand-1 border border-brand-1/30"
        ...
        <h2
          class="text-xl md:text-2xl font-semibold text-text-1 leading-relaxed"
        >
          {{ question.question }}
        </h2>
      </div>
      ...
      <div class="animate-fade-in-delay-2">
        ...
        <div v-if="userMode === 'host'" class="space-y-2 mobile:space-y-3">
          <AnswerButton
        ...
        <div
          v-else-if="userMode === 'participant'"
          class="space-y-2 mobile:space-y-4"
        >
          <ol class="list-none space-y-2 mobile:space-y-3">
            <li
              v-for="(answer, index) in question.answers"
              :key="`participant-answer-${index}`"
              class="flex items-start gap-3 p-3 mobile:p-4 bg-bg-soft/50 rounded-lg border border-divider/50"
            >
              <span
                class="flex-shrink-0 w-6 h-6 bg-brand-1 text-white rounded-full flex items-center justify-center text-sm font-semibold"
              >
                {{ index + 1 }}
              </span>
              <span
                class="flex-1 text-base leading-6 mobile:text-lg text-text-1"
              >
                {{ answer }}
              </span>
            </li>
        ...
        <div v-else class="space-y-2 mobile:space-y-3">
          <AnswerButton
            v-for="(answer, index) in question.answers"
      ...
      <div
        v-if="userMode === 'host' && hasTeamAssignments"
        class="bg-bg rounded-lg p-4 border border-divider animate-fade-in-up mt-6"
      >
        <div class="flex justify-between items-center mb-3">
          <span class="font-medium text-text-1">Attribution des équipes</span>
          <span
            class="text-sm font-medium text-brand-1 bg-bg-soft border border-brand-1/30 px-2 py-1 rounded"
            >{{ assignedTeamsCount }}/{{ totalTeams }}</span
          >
        </div>
        <div class="space-y-2">
          <div
            v-for="[answerIndex, teams] in Array.from(
              teamAssignments || new Map()
            )"
            :key="answerIndex"
            class="flex items-center justify-between p-2 bg-bg-soft rounded border border-divider"
          >
            <span class="text-sm font-medium text-text-1 flex-1 mr-3">{{
              question.answers[answerIndex]
            }}</span>
            <div class="flex flex-wrap gap-1">
              <span
                v-for="teamId in teams"
                :key="teamId"
                class="inline-flex items-center px-2 py-1 rounded text-xs font-medium bg-bg-soft text-brand-2 border border-brand-2/30"
              >
                {{ getTeamName(teamId) }}
              </span>
            </div>
      ...
      <div class="animate-fade-in-up mt-6">
        ...
        <div v-else-if="userMode === 'host'">
          ...
          <div
            v-else
            class="text-center p-4 bg-bg rounded-lg border border-divider"
          >
            <p class="text-text-2 mb-2">
              Attribuez une réponse à chaque équipe pour révéler la bonne
              réponse
            </p>
            <div class="flex justify-center">
              <span class="text-sm font-medium text-brand-1"
                >{{ assignedTeamsCount }}/{{ totalTeams }} équipes
                assignées</span
              >
            </div>
          </div>
          ...
          >
            <div class="flex items-center">
              <span
                ...
                class="text-green-1 font-medium flex items-center gap-1"
              >
                ✅ Bonne réponse !
              </span>
              <span
                v-else
                class="text-red-1 font-medium flex items-center gap-1"
              >
                ❌ Pas tout à fait...
              </span>
            </div>
      ...
      <div
        v-if="shouldShowExplanation && question.explanation"
